...
class StatusDot(Widget):
	def __init__(self, **kwargs):
		...
		self._alpha       = 0.1
		self._going_up    = True     # True = вспышка вверх, False = медленное затухание
		self.size_hint    = (None, None)
		self.size         = (40, 40)

		with self.canvas:
			self._c_glow  = Color(*C.SUCCESS[:3], 0)
			self._e_glow  = Ellipse(pos=self.pos, size=self.size)
			self._c_inner = Color(*C.TEXT_DIM)
			self._e_inner = Ellipse(pos=self.pos, size=self.size)
	...
	def _reposition(self, *a):
		...
		ir = r * 0.52
		self._e_glow.pos   = self.pos
		self._e_glow.size  = (r * 2, r * 2)
		self._e_inner.pos  = (self.x + r - ir, self.y + r - ir)
		self._e_inner.size = (ir * 2, ir * 2)

	def _apply_alpha(self):
		if self._running:
			self._c_glow.rgba  = (*C.SUCCESS[:3], self._alpha * 0.35)
			self._c_inner.rgba = (*C.SUCCESS[:3], self._alpha)
		else:
			self._c_glow.rgba  = (0, 0, 0, 0)
			self._c_inner.rgba = (*C.TEXT_DIM[:3], 1)

	def _pulse_tick(self, dt):
		if self._going_up:
			# Быстрая вспышка — 0.15 сек до максимума
			self._alpha += dt / 0.15
			if self._alpha >= 1.0:
				self._alpha    = 1.0
				self._going_up = False
		else:
			# Медленное затухание — 1.2 сек до минимума
			self._alpha -= dt / 1.2
			if self._alpha <= 0.07:
				self._alpha    = 0.07
				self._going_up = True
	...
	def set_running(self, val):
		...
		if val:
			...
			self._going_up = True
			self._apply_alpha()
			self._pulse_clock = Clock.schedule_interval(self._pulse_tick, 1 / 180)
		else:
			self._alpha = 1.0
			self._apply_alpha()
...
class Studio(FloatLayout):
	def __init__(self, **kwargs):
		...
		self.hamburger = RoundButton(
			...
			pos_hint={'center_x': 0.5, 'top': 0.98}, font_size=50,
		)
		#self.hamburger.x = 18
